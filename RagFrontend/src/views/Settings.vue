      ...
      <div v-if="activeTab === 'audit'" class="tab-content">
        ...
        <div v-else class="audit-table-wrapper">
          ...
          <div class="pagination">
            <button :disabled="auditPage <= 1" @click="goToPreviousAuditPage">上一页</button>
            <span>第 {{ auditPage }} 页</span>
            <button @click="goToNextAuditPage">下一页</button>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'model-config'" class="tab-content">
        ...
        <div class="mc-installed-bar" v-if="mcLocalModels.length">
          ...
          <span
            ...
            :key="m"
            class="mc-model-chip"
            @click="applyLocalModelSelection(m)"
            title="点击快速填入"
            >{{ m }}</span
          >
        </div>
        ...
        <div class="mc-form">
          ...
          <div class="mc-form-row">
            ...
            <input
              ...
              class="mc-input"
              placeholder="如：当前已安装模型 / llama3:8b"
            />
          ...
          <div v-if="activePlatform" class="platform-config-panel">
            ...
            <template v-if="activePlatform === 'obsidian'">
              ...
              <div class="form-actions">
                <button class="btn-primary" @click="configObsidian" :disabled="obsidianLoading">
                  配置 Vault
                </button>
                <button
                  class="btn-secondary"
            ...
            <template v-else-if="activePlatform === 'feishu'">
              ...
              <div class="form-actions">
                <button class="btn-primary" @click="configFeishu" :disabled="feishuLoading">
                  保存配置
                </button>
              </div>
...
function applyLocalModelSelection(modelName: string) {
  mcForm.llm_model = modelName
  mcForm.kg_model = modelName
}
...
function handleLayoutChange(layoutId: string) {
  appearance.layout = layoutId as typeof appearance.layout
  saveAppearance()
}
// ── 使用统计 ──────────────────────────────────────────────────
