...
const embedCode = computed(
  () =>
    `<iframe src="${shareLink.value}/embed" width="100%" height="400" frameborder="0" title="${props.kb?.name || 'Knowledge Base'}"></iframe>`
)
...
function shareToApp(app: any) {
  ...
  if (app.id === 'email') {
    window.open(
      `mailto:?subject=${encodeURIComponent('分享知识库：' + (props.kb?.name || ''))}&body=${encodeURIComponent(shareLink.value)}`
    )
    return
  }
