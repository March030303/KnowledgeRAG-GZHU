...
export default function SquareScreen() {
  ...
  const fetchKbs = useCallback(
    async (reset: boolean, keyword: string, cat: string, sort: string, pg: number) => {
      try {
  ...
  const handleStar = useCallback(async (kb: SharedKb) => {
    try {
      ...
      setKbList(prev =>
        prev.map(k =>
          k.id === kb.id ? { ...k, star_count: k.star_count + (res.data.starred ? 1 : -1) } : k
        )
      )
    } catch {
      /* 静默 */
    ...
    try {
      if (circle._joined) {
        ...
        setCircles(prev =>
          prev.map(c =>
            c.id === circle.id
              ? { ...c, _joined: false, member_count: Math.max(1, c.member_count - 1) }
              : c
          )
        )
      } else {
