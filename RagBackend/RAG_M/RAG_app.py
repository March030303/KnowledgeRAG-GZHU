...
try:
    ...
    _REACT_AGENT_IMPORT_ERROR = None
except Exception as exc:
    ReActRAGAgent = None  # type: ignore[assignment]
    _REACT_AGENT_IMPORT_ERROR = exc
    logger.warning("[RAG_app] ReAct Agent 模块加载失败，相关接口将暂时不可用: %s", exc)
...
async def rag_health_check():
    ...
    return {
        ...
        "vectorstore_path": vectorstore_path,
        "vectorstore_exists": (
            os.path.exists(vectorstore_path) if vectorstore_path else False
        ),
        "features": [
            "hybrid_retrieval",
            "bm25+vector+rrf",
...
class AgentQueryRequest(BaseModel):
    ...
    model: Optional[str] = (
        None  # 云端模型：cloud:provider:model_id，Ollama 直接传 model_name
    )
...
async def agent_query(query_body: AgentQueryRequest):
    ...
    if ReActRAGAgent is None:
        raise HTTPException(
            status_code=503,
            detail=f"ReAct Agent 当前不可用：{_REACT_AGENT_IMPORT_ERROR}",
        )
...
async def native_query(req: NativeQueryRequest):
    ...
    async def generate():
        try:
            ...
            from src.rag.native_rag import NativeRAGPipeline, NativeVectorStore
            ...
            if is_cloud:
                ...
                try:
                    ...
                    logger.warning(
                        f"[RAG_app] 读取用户模型配置失败，使用环境变量: {_cfg_err}"
                    )
