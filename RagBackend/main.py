...
def _apply_env_compat_aliases() -> None:
    """Propagate legacy / alternate env names so mixed configs still work."""
    groups = [
...
# Configure structured logging. Each log carries trace_id for full traceability.
from exception.exceptions import APIException, api_exception_handler
from trace_logging import TraceMiddleware, get_trace_id, setup_trace_logging
...
async def _init_db_tables():
    ...
    logger.info(
        "运维可观测性已启用：docs=%s openapi=%s audit_logs=%s audit_stats=%s metrics=%s",
        "/docs",
        "/openapi.json",
        "/api/audit/logs",
        "/api/audit/stats",
...
from document_processing.doc_list import (
    router as doc_list,  # Import file list service interface
)
from document_processing.doc_manage import (
    router as doc_manage,  # Import file processing service interface
)
from document_processing.doc_upload import (
    router as upload_models,  # Import file upload service interface
)
...
from knowledge_base.knowledgeBASE4CURD import (
    router as knowledge_CURD,  # Import knowledge base CRUD interface
)
from knowledge_base.knowledgebase_cover import (
    router as pic_cover_manage,  # Import cover image management interface
)
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        ...
        content={
            "code": "500001",
            "message": "服务器内部错误",
            "detail": str(exc) if app.debug else "请联系运维并提供 trace_id",
            "trace_id": trace_id,
        },
    )
