    ...
    try:
        knowledge_bases.sort(
            key=lambda x: datetime.strptime(
                x.get("created_time", ""), "%Y-%m-%d %H:%M:%S"
            ),
            reverse=True,
        )
        logger.debug("知识库排序成功")
...
async def create_knowledgebase(request: KnowledgeBaseCreate) -> KnowledgeBaseResponse:
    """
    创建新知识库
    ...
    try:
        ...
        return KnowledgeBaseResponse(
            code="000000", message="知识库创建成功", data=kb_data
        )
...
async def delete_knowledgebase(kb_id: str) -> JSONResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="没有权限删除该知识库"
        )
    except Exception as e:
...
async def get_knowledge_items(
    ...
    try:
        ...
        return KnowledgeBaseListResponse(
            code="000000", message="获取知识库列表成功", data=data, total=len(data)
        )
...
async def get_knowledge_item_by_id(kb_id: str) -> KnowledgeBaseResponse:
    ...
    try:
        ...
        return KnowledgeBaseResponse(
            code="000000", message="获取知识库成功", data=kb_data
        )
...
@router.post(
    "/update-knowledgebase-config/{kb_id}", response_model=KnowledgeBaseResponse
)
async def update_knowledgebase_config(
    kb_id: str, request: KnowledgeBaseUpdateRequest
) -> KnowledgeBaseResponse:
    """
    更新知识库配置
    ...
    try:
        ...
        return KnowledgeBaseResponse(
            code="000000", message="知识库配置已更新", data=kb_data
        )
...
async def list_knowledge_bases_alias(user_id: Optional[str] = None) -> List[dict]:
    """
