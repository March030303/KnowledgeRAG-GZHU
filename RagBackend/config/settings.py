...
class BaseConfig:
    ...
    KB_STORAGE_PATH = os.getenv(
        "KB_STORAGE_PATH",
        str(PROJECT_ROOT / "RagBackend" / "local-KLB-files")
    )
    ...
    DEFAULT_LLM_MODEL = os.getenv("DEFAULT_LLM_MODEL", "qwen2:0.5b")
...
class ProductionConfig(BaseConfig):
    ...
    TESTING = False
    # 生产环境需要显式设置敏感配置
    if not os.getenv("JWT_SECRET_KEY") or os.getenv("JWT_SECRET_KEY") == "your-secret-key-change-in-production":
        raise ValueError("⚠️ 生产环境必须设置 JWT_SECRET_KEY 环境变量")
