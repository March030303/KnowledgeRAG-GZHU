...
class KnowledgeBaseCreate(BaseModel):
    ...
    owner_id: Optional[str] = Field(
        None, max_length=100, description="所有者ID（可选）"
    )
...
class KnowledgeBaseConfig(BaseModel):
    ...
    embedding_model: str = Field(
        "sentence-transformers/all-MiniLM-L6-v2", description="嵌入模型"
    )
    chunk_size: int = Field(1000, ge=100, le=4000, description="文档分块大小（字符数）")
    chunk_overlap: int = Field(200, ge=0, le=1000, description="文档分块重叠（字符数）")
...
class KnowledgeBaseKGConfig(BaseModel):
    ...
    selected_entity_types: List[str] = Field(
        ["PERSON", "ORGANIZATION", "LOCATION"], description="要提取的实体类型"
    )
...
class KnowledgeBaseSchema(
    KnowledgeBaseMetadata, KnowledgeBaseConfig, KnowledgeBaseKGConfig
):
...
class KnowledgeBaseUpdateRequest(BaseModel):
    ...
    similarity_threshold: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="相似度阈值"
    )
