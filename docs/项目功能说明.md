...
```
前端选择文件 → 分块（每块默认 0.1MB/100KB）→ POST /api/upload-chunk/ → POST /api/upload-complete/ → 合并 → 元数据写入
```
...
> `model` 字段可选；支持本地 Ollama 模型（如 `"llama3:8b"`）或 `"cloud:deepseek:deepseek-chat"`（云端）格式。
...
用户输入 → 前端附加历史 → POST /api/chat/send-message
         ...
         → 返回 { reply, model, provider }
         → 前端渲染 Markdown + 引用溯源气泡
```

> `provider` 字段值：`"ollama"`（本地）/ `"deepseek"` / `"openai"` / `"hunyuan"` 等云端标识。
...
| 场景             | 推荐模型                                 | 内存需求 |
| ---------------- | ---------------------------------------- | -------- |
| 低配环境（≤8GB） | 小参数本地模型（如 `llama3.2:1b`，~1GB） | ~1GB     |
| 中配环境（16GB） | 标准本地模型（如 `llama3:8b`，4GB+）     | ~8GB     |
...
| GET    | `/api/get-knowledge-item/`              | 获取知识库列表（可选 `?user_id=` 过滤）   |
| GET    | `/api/get-knowledge-item/{id}`          | 获取单个知识库信息                        |
| GET    | `/api/list-knowledge-bases/`            | 获取知识库列表（简洁格式别名）            |
| POST   | `/api/update-knowledgebase-config/{id}` | 更新知识库配置（名称/描述/分块/解析器等） |
| DELETE | `/api/delete-knowledgebase/{id}`        | 删除知识库（同步清理本地文件）            |
...
| Method | Path                 | 功能                                                          |
| ------ | -------------------- | ------------------------------------------------------------- |
| POST   | `/api/ollama-models` | Ollama 官网模型列表（1h 缓存，POST body: `{ "page_id": 1 }`） |
| GET    | `/`                  | 服务健康检查                                                  |
| GET    | `/helloworld/`       | 测试端点                                                      |
...
| 配置项         | 环境变量          | 默认值                                   | 说明                         |
| -------------- | ----------------- | ---------------------------------------- | ---------------------------- |
| LLM 模型       | `MODEL`           | `deepseek-chat`                          | 用于 RAG 问答 / Agent / 聊天 |
...
# 拉取本地模型（低配环境推荐小参数模型）
ollama pull llama3.2:1b    # 示例，也可换其他已支持的模型
...
| Ollama 500 内存不足 | 模型需求内存过高     | 切换更小的模型后重启后端                  |
